...
function JobPostingList({ jobPost, type }) {
    ...
    const JobPostingCard = ({ isServiceProvider, job }) => {
        const imageUrl = job.image ? URL.createObjectURL(new Blob([job.image.buffer])) : null;
        ...
        const isApplied = requestsByTaskId[job._id]?.some(request => 
            request.userId === user._id && requestsByTaskId[job._id].length > 0
        );
                    ...
                    <div className="w-full flex flex-col gap-2">
                        <div className="flex justify-between">
                            ...
                            <h2 className="font-bold">
                                ...
                                {subCategories[job._id]?.subCategoryName || "Loading SubCategory..."}
                            </h2>
                        ...
                        {isApplied && <span className="text-primary">isApplied</span>}
                    </div>
                </div>
            </div>
        );
                    ...
                    <div key={job._id}>
                        {job.status === "created" ? (
                            user && user.role === "normal" ? (
                                ...
                                user.city?.trim().toLowerCase() === job.city?.trim().toLowerCase() && (
